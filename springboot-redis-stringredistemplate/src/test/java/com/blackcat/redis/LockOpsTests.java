...
class LockOpsTests {
    ...
    void releaseLockTest() {
        ...
        try {
             ...
             */
            lock = RedisStringUtil.LockOps.getLock(lockName, lockValue);
        } finally {
            ...
            if (lock) {
                 ...
                 * releaseLock(...) => result -> true
                 */
                RedisStringUtil.LockOps.releaseLock(lockName, lockValue);
            }
        }
    }
    ...
    void getLockUntilTimeoutTest() {
        ...
        RedisStringUtil.LockOps.getLock(lockName, lockValue, 300, TimeUnit.SECONDS);
         ...
         */
        RedisStringUtil.LockOps.getLockUntilTimeout(lockName, lockValue, 2, TimeUnit.SECONDS, 5000);
    }
}